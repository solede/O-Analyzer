...
Public Class CLS_ODBC
    ...
    'グローバル変数
    Public G_CONN As New OdbcConnection 'オラクルコネクション
    Public G_CONN2 As Object = G_CONN 'G_CONNを参照する変数
    Public G_TRAN As OdbcTransaction

    'モジュール変数
    Private M_READER As OdbcDataReader
    Private M_ADAPTER As OdbcDataAdapter
    Private M_PARAMETERS() As OdbcParameter
    ...
    Function DB_CONNECT(ByVal W_USERID As String, ByVal W_PASS As String, ByVal W_CONN_STR As String, Optional ByVal W_OPTION As String = vbNullString, Optional ByVal W_SYSDBA_FLG As Boolean = False, Optional ByVal W_OUTPUT_ERR As Boolean = True, Optional ByVal logName As String = vbNullString) As Boolean
        ...
        If W_USERID = vbNullString And W_PASS = vbNullString Then
            'OS認証
            If InStr(W_CONN_STR, "\") > 0 Then
                connStr = "UID= / " & ";FILEDSN=" + W_CONN_STR & ";" & W_OPTION
            Else
                connStr = "UID= / " & ";DSN=" + W_CONN_STR & ";" & W_OPTION
            End If
        ElseIf W_SYSDBA_FLG And W_PASS = vbNullString Then
            'SYSDBA接続(パスワード入力無(OS認証))
            If InStr(W_CONN_STR, "\") > 0 Then
                connStr = "UID= / " & ";FILEDSN=" + W_CONN_STR & ";DBA Privilege=SYSDBA;" & W_OPTION
            Else
                connStr = "UID= / " & ";DSN=" + W_CONN_STR & ";DBA Privilege=SYSDBA;" & W_OPTION
            End If
        ElseIf W_SYSDBA_FLG And W_PASS <> vbNullString Then
            'SYSDBA接続(パスワード入力有(OS認証＋パスワード認証))
            If InStr(W_CONN_STR, "\") > 0 Then
                connStr = "UID=" & W_USERID & ";PWD=" & W_PASS & ";FILEDSN=" & W_CONN_STR & ";DBA Privilege=SYSDBA;" & W_OPTION
            Else
                connStr = "UID=" & W_USERID & ";PWD=" & W_PASS & ";DSN=" & W_CONN_STR & ";DBA Privilege=SYSDBA;" & W_OPTION
            End If
        Else
            '通常接続
            If InStr(W_CONN_STR, "\") > 0 Then
                connStr = "UID=" & W_USERID & ";PWD=" & W_PASS & ";FILEDSN=" & W_CONN_STR & ";" & W_OPTION
            Else
                connStr = "UID=" & W_USERID & ";PWD=" & W_PASS & ";DSN=" & W_CONN_STR & ";" & W_OPTION
            End If
        End If
        ...
        Catch ex As Exception
            If W_OUTPUT_ERR Then
                MsgBox(ex.Message)
            End If
            M_FILE.WriteApLog("接続文字列の定義に失敗しました。ミドルウエア:" & M_CONST.GetMiddlewareName(CLS_CONST.CLS_CONST.C_MIDDLE_TYPE.ODBC) & " 接続文字列:" & connStr.Replace("PWD=" & W_PASS, "PWD=XXXX") & " エラーメッセージ：" & ex.Message, , logName)
            Return False
        End Try
        ...
        Try
            G_CONN.Open()
            G_CONN2 = G_CONN
            M_FILE.WriteApLog("接続に成功しました。ミドルウエア:" & M_CONST.GetMiddlewareName(CLS_CONST.CLS_CONST.C_MIDDLE_TYPE.ODBC) & " 接続定義:" & G_CONN.ConnectionString.Replace("PWD=" & W_PASS, "PWD=XXXX"), , logName)
            Return True
        Catch ex As OdbcException
            If W_OUTPUT_ERR Then
                MsgBox(ex.Message)
            End If
            M_FILE.WriteApLog("接続に失敗しました。ミドルウエア:" & M_CONST.GetMiddlewareName(CLS_CONST.CLS_CONST.C_MIDDLE_TYPE.ODBC) & " 接続定義:" & G_CONN.ConnectionString.Replace("PWD=" & W_PASS, "PWD=XXXX") & " エラーメッセージ：" & ex.Message, , logName)
            Return False
        Catch ex2 As Exception
            If W_OUTPUT_ERR Then
                MsgBox(ex2.Message)
            End If
            CLS_FILE.CLS_FILE.WriteErrLog(ex2, , logName)
            Return False
        End Try
    End Function
    ...
    Function GET_COMPETENCE() As Boolean
        ...
        '  FALSE :参照失敗
        '#######################################################################
        Dim W_CMD As New OdbcCommand
        Dim W_READER As OdbcDataReader
        ...
        Try
            'V$VIEWの検索チェック
            W_CMD.CommandText = "SELECT 0 FROM V$INSTANCE ROWNUM = 1"
            ...
            'ディクショナリの検索チェック
            W_CMD.CommandText = "SELECT 0 FROM DBA_TABLES ROWNUM = 1"
        ...
        Catch ex As OdbcException
            If InStr(ex.Message, "ORA-00942") <> 0 Then
                'ORA-942: 表またはビューが存在しません。
                Return False
            ElseIf InStr(ex.Message, "ORA-01219") <> 0 Then
                'ORA-01219: データベースがオープンしていません
                Return True
            Else
                CLS_FILE.CLS_FILE.WriteErrLog(ex)
                MsgBox(ex.Message)
                Return False
            End If
        Catch ex As Exception
    ...
    Sub DB_CLOSE(Optional ByVal logName As String = vbNullString)
        ...
        '#######################################################################
        Try
            'OdbcConnection.ClearPool(G_CONN)
            G_CONN.Close()
            G_CONN.Dispose()
            CLS_FILE.CLS_FILE.WriteApLog("コネクションを切断しました", , logName)
        Catch ex As Exception
            CLS_FILE.CLS_FILE.WriteApLog("コネクションの切断に失敗しました", , logName)
        End Try
    End Sub
    ...
    Public Function GET_DATA_ADAPTER(ByVal W_SQL As String, Optional ByVal W_TIMEOUT_VALUE As Integer = 0) As Boolean
        '#######################################################################
        'SELECTデータの非接続型取得
        '　odbcDataAdapter⇒DataSetに変換したものを参照渡しで戻す
        '引数
        '  W_SQL           :SQL文
        ...
        '  FALSE :失敗
        '#######################################################################
        Dim W_CMD As New OdbcCommand(W_SQL, G_CONN)
        ...
        Try
            ...
            M_ADAPTER = New OdbcDataAdapter(W_CMD)
            stopWatch.Stop()
    ...
    Public Function GET_DATASET(ByVal W_SQL As String, ByRef W_DATASET As DataSet, Optional ByVal W_TIMEOUT_VALUE As Integer = 0, Optional ByVal W_ERR_DISPLAY As Boolean = True, Optional ByVal logName As String = vbNullString) As Boolean
        '#######################################################################
        'SELECTデータの非接続型取得
        '　SELECT文の結果セットをodbcDataAdapterで取得し、datasetで戻す
        '引数
        '  W_SQL           :SQL文
        '  W_DATASET       :DataSetオブジェクト(参照戻し)
        '  W_TIMEOUT_VALUE :タイムアウト値(秒)
        ...
        Try
            ...
            stopWatch = stopWatch.StartNew()
            W_DA = New OdbcDataAdapter(W_CMD)
        ...
        Catch ex As Exception
            If W_ERR_DISPLAY Then
                MsgBox(ex.Message)
            End If
            CLS_FILE.CLS_FILE.WriteErrLog(ex, , logName)
            W_DATASET = Nothing
            GET_DATASET = False
        End Try
    End Function

    Public Function SET_ODBCDATAREADER(ByVal W_SQL As String, Optional ByVal W_TIMEOUT_VALUE As Integer = 0, Optional ByVal W_OUTPUT_ERR As Boolean = True, Optional ByVal logName As String = vbNullString) As Boolean
        '#######################################################################
        'SELECTデータの接続型取得
        '　SELECT文の結果セットをodbcDataReaderオブジェクトで戻す
        '引数
        '  W_SQL              :SQL文
        '  W_TIMEOUT_VALUE    :タイムアウト値(秒)
        ...
        '  FALSE :失敗
        '#######################################################################
        Dim W_CMD As OdbcCommand
        Dim stopWatch As Stopwatch

        M_READER = Nothing

        Try
            W_CMD = New OdbcCommand(W_SQL, G_CONN)
            'SQLタイムアウトの設定
            'system.data.odbccommandはcommandtimeoutをサポートしていない(設定しても効かない)
            If W_TIMEOUT_VALUE > 0 Then
                W_CMD.CommandTimeout = W_TIMEOUT_VALUE
            End If
        ...
        For W_TEMP_COL_CNT = 0 To W_COLUMN_COUNT - 1
            ...
            If Not M_READER.IsDBNull(W_TEMP_COL_CNT) Then
                ...
                ElseIf W_TYPE(W_TEMP_COL_CNT) = CLS_CONST.CLS_CONST.C_COLUMN_TYPE.NUMBER Then
                    W_VALUES(W_TEMP_COL_CNT) = M_READER.GetDouble(W_TEMP_COL_CNT).ToString
                ...
                ElseIf W_TYPE(W_TEMP_COL_CNT) = CLS_CONST.CLS_CONST.C_COLUMN_TYPE.BLOB Then
                    ...
                    W_VALUES(W_TEMP_COL_CNT) = "BLOB:未対応"
                ElseIf W_TYPE(W_TEMP_COL_CNT) = CLS_CONST.CLS_CONST.C_COLUMN_TYPE.RAW Then
                    'W_VALUES(W_TEMP_COL_CNT) = BitConverter.ToString(M_READER.GetBytes(W_TEMP_COL_CNT,0,,0,).Value, 0).Replace("-", "")
                    W_VALUES(W_TEMP_COL_CNT) = "RAW:未対応"
                ElseIf W_TYPE(W_TEMP_COL_CNT) = CLS_CONST.CLS_CONST.C_COLUMN_TYPE.BFILE Then
                    ...
                    W_VALUES(W_TEMP_COL_CNT) = "BFILE:未対応"
    ...
    Public Function SET_BIND_VARIABLE(ByVal W_BIND() As M_CONST.S_BIND) As Boolean
        ...
        For i As Integer = 0 To W_BIND.Length - 1
            M_PARAMETERS(i) = New OdbcParameter(W_BIND(i).GET_NAME, W_ORATYPE(i))
    ...
    Private Sub SET_RESULT_TYPE(ByVal W_COLUMN_COUNT As Integer)
        ...
        For W_TEMP_COL_CNT = 0 To W_COLUMN_COUNT - 1
            ...
            ElseIf W_TYPE(W_TEMP_COL_CNT).ToUpper = "INT16" Or W_TYPE(W_TEMP_COL_CNT).ToUpper = "INT32" Or _
               ...
               W_TYPE(W_TEMP_COL_CNT).ToUpper = "NUMBER" Or W_TYPE(W_TEMP_COL_CNT).ToUpper = "DOUBLE PRECISION" Then
            ...
            ElseIf W_TYPE(W_TEMP_COL_CNT).ToUpper = "XMLTYPE" Then
                M_TYPE(W_TEMP_COL_CNT) = CLS_CONST.CLS_CONST.C_COLUMN_TYPE.STRING
            Else
                CLS_FILE.CLS_FILE.WriteApLog("予期せぬ型がSELECTされました。 列:" & M_READER.GetName(W_TEMP_COL_CNT) & " 型:" & W_TYPE(W_TEMP_COL_CNT))
